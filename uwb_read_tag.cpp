...
vec3d *anchorArray =new vec3d;
int  *distanceArray = new int;
vec3d * mypo = new vec3d;
int result;
int GetLocation(vec3d *best_solution, int use4thAnchor, vec3d* anchorArray, int *distanceArray);
std::ofstream output;
unsigned char buf[128];//定义字符串长度
unsigned char buf_a[128];
unsigned char buf_af[128];
unsigned char last_buf_a[128];
unsigned char yanchuanguwb[130];
unsigned char last_yanchuanguwb[130];
unsigned char yanchuanguwbf[130];
int buf_rev[10];
double trans_data(uint8_t *B,int b_start,int b_n);
int rev_flag=false;
int i=0,j,k;//loop counter
int verify=true,error55=0;
long counter=0;
double num_rev=0.0;
float px; float py; float pz;float px100; float py100; float pz100;
int px_front,px_latter,py_front,py_latter,pz_front,pz_latter;
float pz_array[10];
float pz_sum;
mavros_msgs::PositionTarget uwb_position;
char sender[10]; string sender_;
int loop_trans;
int destination=3;
bool verify_=true;
int yanchuangloopp=2;
float dis0,dis1,dis2,dis3;


float decimal_fliter(float n)
{

}

void switch_commandCB(const mavros_msgs::PositionTarget::ConstPtr& msg, mavros_msgs::PositionTarget* PositionTarget)
{
  *PositionTarget = *msg;
    //just update do nothing
}

void localCB(const nav_msgs::Odometry::ConstPtr& msg, nav_msgs::Odometry* PositionTarget)
{
  *PositionTarget = *msg;
    //cout<<PositionTarget->pose.pose.position.x<<endl;
    //just update do nothing
}
...
int main(int argc,char **argv)
{
	...
	ros::NodeHandle n;
        ros::Publisher pose_pub = n.advertise<mavros_msgs::PositionTarget>("uwb_position",1000);
        //subscriber switch command
        mavros_msgs::PositionTarget switchcommand;
        ros::Subscriber switchcommand_sub =
                  n.subscribe<mavros_msgs::PositionTarget>("switch_command", 1000 , boost::bind(switch_commandCB, _1, &switchcommand));

        nav_msgs::Odometry local;
        ros::Subscriber local_sub =
            n.subscribe<nav_msgs::Odometry>("/mavros/global_position/local", 10, boost::bind(localCB, _1, &local));
	...
	sp.set_option(serial_port::character_size());
        ...
        io_service uwb;
        serial_port sq(uwb,"/dev/ttyACM0");//定义传输的串口
        sq.set_option(serial_port::baud_rate(115200));
        sq.set_option(serial_port::flow_control());
        sq.set_option(serial_port::parity());
        sq.set_option(serial_port::stop_bits());
        sq.set_option(serial_port::character_size());

        output.open("/home/lift1/catkin_ws/src/uwb_read_tag/src/output/output.txt");
	output<<"openandinit-ok"<<endl;
int use4anchor=1;
	while(ros::ok())
        {
                //read(sp,buffer(buf_a));
//get position and tell offboard
//                px = trans_data(buf_a,4,3);
//                py = trans_data(buf_a,7,3);
//                dis0 = trans_data(buf_a,25,3);
//                pz = dis0*dis0 - px*px - (15-py)*(15-py);
//                px = local.pose.pose.position.x+5;
//                py = local.pose.pose.position.y+5+5;
//                pz = local.pose.pose.position.z;
                cout<<"read!"<<endl;
                read(sq,buffer(yanchuanguwb));
                for(int yanchuangloop=0; yanchuangloop<129;yanchuangloop++)
                {
                    //cout<<yanchuanguwb[yanchuangloop]<<" ";
                    if(yanchuanguwb[yanchuangloop]=='m' && yanchuanguwb[yanchuangloop+1]=='c')
                    {

                        for(yanchuangloopp=0;yanchuangloopp<65;yanchuangloopp++)
                        {
                            if(yanchuangloop>129) yanchuangloop=0;
                            yanchuanguwbf[yanchuangloopp]=yanchuanguwb[yanchuangloop-1];
                            yanchuangloop++;

                        }
                    }
                }
                for(int yanchuangloop=0; yanchuangloop<65;yanchuangloop++)
                {
                    //cout<<yanchuanguwbf[yanchuangloop]<<" ";
                }
                cout<<"dis0 = "<<setprecision(12)<<trans_data(yanchuanguwbf,10,4)<<endl;
                cout<<"dis1 = "<<setprecision(12)<<trans_data(yanchuanguwbf,19,4)<<endl;
                cout<<"dis2 = "<<setprecision(12)<<trans_data(yanchuanguwbf,28,4)<<endl;
                cout<<"dis3 = "<<setprecision(12)<<trans_data(yanchuanguwbf,37,4)<<endl;

                dis0=trans_data(yanchuanguwbf,10,4);
                dis1=trans_data(yanchuanguwbf,19,4);
                dis2=trans_data(yanchuanguwbf,28,4);
                dis3=trans_data(yanchuanguwbf,37,4);
...
//                anchorArray[3].z=1.0;
                 ...
                 anchorArray[0].x=0.0;
                 anchorArray[0].y=0.0;
                 anchorArray[0].z=1.79;

                 anchorArray[1].x=0.0;
                 anchorArray[1].y=15.9;
                 anchorArray[1].z=1.79;

                 anchorArray[2].x=15.9;
                 anchorArray[2].y=15.9;
                 anchorArray[2].z=1.79;

                 anchorArray[3].x=15.9;
                 anchorArray[3].y=0.0;
                 anchorArray[3].z=5.57;
distanceArray[0]=dis0;
distanceArray[1]=dis1;
distanceArray[2]=dis2;
distanceArray[3]=dis3;

                result = GetLocation(mypo, use4anchor , anchorArray , distanceArray ) ;
                cout<<"result "<<result<<endl;
                cout<<mypo->x<<"  "<<mypo->y<<"  "<<local.pose.pose.position.z<<endl;
                px=mypo->x;py=mypo->y;pz=local.pose.pose.position.z;
//                if(pz<=0) pz=-pz;
//                pz=sqrt(pz);
//                for(i=9;i>0;i--)
//                {
//                    pz_array[i]=pz_array[i-1];
//                }
//                pz_array[0]=pz;
//                pz_sum = 0;
//                for(i=0;i<10;i++)
...
//                pz=pz_sum/10;
                ...
                //px=0.44+counter%5; py=0.55+counter%5; pz=0.66+counter%5;
                px100=px*100; py100=py*100; pz100=pz*100;
                px100=(int)px100;
                py100=(int)py100;
                pz100=(int)pz100;
                ...
                px_front=(int)px;
                if(px_front == 0)
                    sender[0]=(char)(px_front+60);
                else
                    sender[0]=(char)(px_front);
                px_latter = px100 - px_front*100; sender[1]=(char)px_latter;

                py_front=(int)py;
                if(py_front == 0)
                    sender[2]=(char)(py_front+60);
                else
                    sender[2]=(char)(py_front);
                py_latter = py100 - py_front*100; sender[3]=(char)py_latter;

                pz_front=(int)pz;
                if(pz_front == 0)
                    sender[4]=(char)(pz_front+60);
                else
                    sender[4]=(char)(pz_front);
                pz_latter = pz100 - pz_front*100; sender[5]=(char)pz_latter;

                sender[6]=(char)switchcommand.position.y;//who i am
                sender[7]=(char)switchcommand.position.x;//where i'll go

                sender_ = sender;
                write (sp,buffer(sender_,10));
                uwb_position.position.x=px;
                uwb_position.position.y=py;
                uwb_position.position.z=pz;

                pose_pub.publish(uwb_position);
                output<<px<<"  "<<py<<"  "<<pz<<endl;
//boardcast position

                //switch send
//                cout<<"switchcommand.position.x = "<<switchcommand.position.x<<endl;
//                loop_trans=(int)switchcommand.position.x;
//                sender[6] =(char)loop_trans;


/*
                //该标签与各个基站的距离 m
		int tag_id=trans_data(buf,2,1); //cout<<"tag_id  "<<tag_id<<endl;
		msg.data=tag_id;
                double dis0 = trans_data(buf,22,3);	//cout<<"dis0  "<<dis0<<endl;
                double dis1 = trans_data(buf,25,3);
                double dis2 = trans_data(buf,28,3);
                double dis3 = trans_data(buf,31,3);
                double dis4 = trans_data(buf,34,3);
                double dis5 = trans_data(buf,37,3);
                double dis6 = trans_data(buf,40,3);
                double dis7 = trans_data(buf,43,3);

                //该标签的位置 m
                double tag_position_x = trans_data(buf,4,3);
                double tag_position_y = trans_data(buf,7,3);
                double tag_position_z = trans_data(buf,10,3);

                //标签的速度 m/s
                double tag_velocity_x = trans_data(buf,13,3);
                double tag_velocity_y = trans_data(buf,16,3);
                double tag_velocity_z = trans_data(buf,19,3);

                //三轴陀螺仪的角速度 rad/s
                double gyro_x = trans_data(buf,46,4);
                double gyro_y = trans_data(buf,50,4);
                double gyro_z = trans_data(buf,54,4);

                //三轴加速度 m/s^2
                double acc_x = trans_data(buf,58,4);
                double acc_y = trans_data(buf,62,4);
                double acc_z = trans_data(buf,66,4);
		
/*		uwb_data_ros.dis0_uwb = dis0;
                uwb_data_ros.dis1_uwb = dis1;
                uwb_data_ros.dis2_uwb = dis2;
                uwb_data_ros.dis3_uwb = dis3;
                uwb_data_ros.dis4_uwb = dis4;
                uwb_data_ros.dis5_uwb = dis5;
                uwb_data_ros.dis6_uwb = dis6;
                uwb_data_ros.dis7_uwb = dis7;
                
                output<<dis0<<","<<dis1<<","<<dis2<<","<<dis3<<endl;

                uwb_data_ros.tag_position_x_uwb = tag_position_x;
                uwb_data_ros.tag_position_y_uwb = tag_position_y;
                uwb_data_ros.tag_position_z_uwb = tag_position_z;

                uwb_data_ros.tag_velocity_x_uwb = tag_velocity_x;
                uwb_data_ros.tag_velocity_y_uwb = tag_velocity_y;
                uwb_data_ros.tag_velocity_z_uwb = tag_velocity_z;

                uwb_data_ros.gyro_x_uwb = gyro_x;
                uwb_data_ros.gyro_y_uwb = gyro_y;
                uwb_data_ros.gyro_z_uwb = gyro_z;

                uwb_data_ros.acc_x_uwb = acc_x;
                uwb_data_ros.acc_y_uwb = acc_y;
                uwb_data_ros.acc_z_uwb = acc_z;
*/
//a rev
//find 0x55 and show data

		if(verify==true)
		{
                    for(i=0;i<127;i++)
                    {
                            last_buf_a[i]=buf_a[i];
                            if(buf_a[i]==0x55)
                                    {
                                            cout<<"find"<<i<<endl;
                                            k=i;
                            ...
                            if(error55!=1)  cout<<"0x55error"<<endl;
                    }
                    verify=false;
		}
		else
		{
				...
				{
					if(i<k)
					buf_af[i]=last_buf_a[k+i];
					else
					buf_af[i]=buf_a[i-k];
				}
				for(i=0;i<127;i++) last_buf_a[i]=buf_a[i];
		}
		//for(int i=0;i<127;i++) cout<<(int)buf_af[i]<<" ";
		cout<<endl;
		//for(j=0;j<i;j++) buf_af[j]=last_buf_a[i];
		//for(i=0;i<895;i++) last_buf_a[i]=last_buf[i];
		if(rev_flag)
		{
			//for(int i=13;i<=22;i++) buf_rev[i-13]=buf_a[i];//10bit rev
			//cout<<"buf_rev   ";	cout<<(char)buf_a[14];
			//cout<<(char)buf_a[15]<<endl;
			for(int i=0;i<895;i++) cout<<(int)buf_a[i]<<" ";
			cout<<endl;
			//cout<<buf_a[0]<<endl;
		}
		...
		//cout<<"buf_rev10  "<<endl;
		//for(int i=0;i<10;i++) cout<<buf_rev[i];
		ros::spinOnce();
		loop_rate.sleep();
        }
	iosev.run();
...
double trans_data(uint8_t *B,int b_start,int b_n)
{
	...
	for(int i = 0;i<b_n;i++)
	{
            ...
            if(d>57)
            {
              ...
              sum=sum+  d*(pow(16,1*(b_n-i)));
                //if(b_start==22)
                //	ROS_INFO("%d",*(B+b_start+i));
...
//	if (b_start<11 || (b_start>21 && b_start<44))
//		result=result/1000;
//	else if(b_start>18 && b_start<20)
//		result=result/10000;
...
int GetLocation(vec3d *best_solution, int use4thAnchor, vec3d* anchorArray, int *distanceArray)
{
    ...
    if(result >= 0)
    {
        ...
        //return error
        return -1;
        //cout<<"bestsolution"<<best_solution->x<<endl;
}
